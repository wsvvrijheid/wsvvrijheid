import { IconButton, Input, InputGroup, InputLeftElement, InputRightElement, useUpdateEffect } from '@chakra-ui/react'
 ...
 */
export const SearchForm = ({ placeholder, delay, onSearch, onReset, mode = 'change' }) => {
  ...
  useUpdateEffect(() => {
    if (mode === 'change' && debouncedSearchTerm.length > 2) onSearch?.(debouncedSearchTerm)
  ...
  return (
    <InputGroup size='lg' flex='1'>
      ...
      <InputRightElement w='max-content' right={1}>
        {searchTerm.length > 1 && (
          <IconButton variant='ghost' icon={<FaTimes color='gray.400' />} onClick={() => setSearchTerm('')} />
        )}
        {mode === 'click' && <IconButton colorScheme='blue' onClick={() => onSearch(searchTerm)} icon={<FaSearch />} />}
      </InputRightElement>
    </InputGroup>
  )
}
